...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    fluidRow(
      ...
      box(title=i18n$t('Thematic Map of Emissions and Thermal Electricity generated by Fuel Type'), status = "primary", solidHeader = TRUE, 
    ...
    fluidRow(
      ...
      box(title=i18n$t('Input(TJ), Electricity generated(TJ), emissions(metric tonnes), cost($ x 1,000), efficiency ratio'), status = "primary", solidHeader = TRUE, 
          collapsible = TRUE,
          column(3,selectizeInput("province", label = HTML(paste0('<FONT color="#55579A"><FONT size="4pt">',i18n$t('Province'))),
                                  choices = areas, selected='British Columbia')),
          ...
          column(3,selectizeInput("commodity2", label = HTML(paste0('<FONT color="#55579A"><FONT size="4pt">',i18n$t('To'))),
                                  choices = setNames(commodities,commodity_labels), selected = 'natural_gas')),
    ...
    fluidRow(box(title = i18n$t("Provincial thermal energy information"), status = "primary", solidHeader = TRUE,
                 collapsible = TRUE, column(3,selectInput("variable",
                                                          ...
                                                          choices = c("Inputs (TJ)"="input","Electricity generated (TJ)"="output","Average cost/unit ($ x 1,000)"= "price","Emissions (tonnes)"='emission'), selected='Inputs (TJ)')),
...
server <- function(input, output) {
  ...
  observe({
    ...
    prov_popup <- paste0('<strong>',selected_com_ind()$NAME,', ',
                         ...
                         '<br><strong>Electricity generated: </strong>',formatC(round(selected_com_ind()$outputs,0), format = 'd', big.mark = ","), " TJ")
  ...
  output$linegraph_input <- renderPlotly({
    ...
    b = plot_ly() %>%
      add_trace(data=linegraph_reactive(),x= ~year, y = ~output, type = 'scatter', mode = 'none', name= toTitleCase(gsub('_', ' ', input$commodity1)),  
                ...
                line = list(color = 'rgba(255, 212, 96, 1)',width = 1),
                hoverinfo = "text",
                text = ~paste(format(round(output,0),big.mark = ",",scientific = FALSE),'TJ'),legendgroup = ~commodity) %>%
      ...
      layout(margin = list(t=75), legend = list(orientation = 'h'), title = paste0('Comparing ',toTitleCase(gsub('_', ' ', input$commodity1)),' to ',toTitleCase(gsub('_', ' ', input$commodity2)),' in ',input$province),
             ...
             yaxis = list(range=c(~min(c(linegraph_reactive()$output,linegraph_reactive2()$output)),~max(c(linegraph_reactive()$output,linegraph_reactive2()$output))),
                          side = 'left', title = i18n$t('Electricity generated (TJ)'), showgrid = FALSE, zeroline = FALSE)) %>% 
      config(displayModeBar = F)
  ...
  output$linegraph2 <- renderPlotly({
    ...
    b = plot_ly() %>%
      add_trace(data=linegraph_reactive(), x= ~year, y = ~efficiency, type = 'scatter', mode = 'none', name = input$commodity1,
                ...
                hoverinfo = "text",
                showlegend=FALSE,
                text = ~paste(format(round(efficiency,1),big.mark = ",",scientific = FALSE),'TJ/TJ')) %>%
      ...
      layout(xaxis = list(title = "",range = c(min(input$year2),max(input$year2))),
             ...
             yaxis = list(range=c(~min(c(linegraph_reactive()$efficiency,linegraph_reactive2()$efficiency)),~max(c(linegraph_reactive()$efficiency,linegraph_reactive2()$efficiency))),
                          side = 'left', title = i18n$t('Efficiency (TJ/TJ)'), showgrid = FALSE, zeroline = FALSE)) %>% 
      config(displayModeBar = F)
  ...
  output$bubble_2 <- renderPlotly({
    ...
    efficiency_selected() %>%
      plot_ly(type = 'scatter', mode = 'markers', x = ~efficiency, y = ~emission, size = ~price, color = ~commodity,
              ...
              text=~paste('<b>',toTitleCase(gsub('_', ' ', commodity)),year,'</b>','<br>',
                          '<b>Cost: </b>$',format(round(price,2),big.mark=",",scientific=FALSE),'<br>',
                          '<b>Efficiency: </b>',format(round(efficiency,1),big.mark=",",scientific=FALSE),'TJ','<br>',
                          '<b>Emissions: </b>',format(round(emission,1),big.mark=",",scientific=FALSE),'tonnes')) %>%
      layout(xaxis = list(title = i18n$t('Efficiency (TJ/TJ)'),zeroline=FALSE),
  ...
  table_reactive <- reactive({
    ...
    subject_matter_2 %>%
      ...
      summarize(mean_value=round(mean(indicator,na.rm=TRUE),0)) %>%
  ...
  output$prov_comp <- renderDataTable({
    ...
    as.datatable(formattable(table_reactive(), align = c("l", rep("r", ncol(table_reactive())-1)),
                             list('Province' = custom_color_tile_2(),
                                  ...
                                  'Heavy fuel oil' = custom_color_tile('white','#8c81f7'),
                                  'Wood' = custom_color_tile('white','#8c81f7'),
                                  'Uranium' = custom_color_tile('white','#8c81f7'),
                                  'Total coal' = custom_color_tile('white','#8c81f7'),
                                  'Natural gas' = custom_color_tile('white','#8c81f7'))),
